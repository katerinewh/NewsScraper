function saveEvent(){
      ...
      $.ajax({
        ...
        for(var i=0; i < data.length; i++){
          ...
          "<tbody><tr id ="+ data[i]._id+" ><td>" +data[i].title+"</td>"+
                  "<td>" + data[i].summary+"</td>"+
                  "<td><button class='btn btn-success articleComments' data-toggle='modal' data-target='#note' data-id="+data[i]._id+" >Article Notes</button></td><td><button class='btn btn-danger delete'>Delete Article</button></td></tr></tbody>"
          );
        }//end of for loop
        deleteArticle();
  ...
  // code below is to pass the article id to the comment-modal so that eventually I can create a comment and link it to the correct article
    $('note').on('show.bs.modal', function(e) {
      ...
      $("note").attr("data-article-id",articleId);
      // console.log(articleId);
      ...
      $.ajax({
        ...
        console.log(data);
        for(var i=0; i<dats.note.length; i++){
